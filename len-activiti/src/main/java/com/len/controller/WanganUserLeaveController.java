 ...
 */
@Controller
@RequestMapping("/wangan/leave")
public class WanganUserLeaveController extends BaseController {

    @Autowired
    WanganUserLeaveService leaveService;
    ...
    public String showUser(Model model) {
        return "/act/wangan/leave/leaveList";
    }

    @GetMapping(value = "showLeaveList")
    @ResponseBody
    public ReType showLeaveList(Model model, WanganUserLeave WanganUserLeave, String page, String limit) {
        String userId = CommonUtil.getUser().getId();
        WanganUserLeave.setUserId(userId);
        List<WanganUserLeave> tList = null;
        Page<WanganUserLeave> tPage = PageHelper.startPage(Integer.valueOf(page), Integer.valueOf(limit));
        try {
            tList = leaveService.selectListByPage(WanganUserLeave);
            for (WanganUserLeave leave : tList) {
    ...
    public String leaveDetail(Model model, String processId) {
        ...
        return "/act/wangan/leave/leaveDetail";
    }

    @GetMapping("addLeave")
    public String addLeave() {
        return "/act/wangan/leave/add-leave";
    }
    ...
    public String updateLeave(Model model, @PathVariable String taskId) {
        ...
        WanganUserLeave leave = leaveService.selectByPrimaryKey(baseTask.getId());
        model.addAttribute("leave", leave);
        model.addAttribute("taskId", taskId);
        return "/act/wangan/leave/update-leave";
    }

    @PostMapping("updateLeave/updateLeave/{taskId}/{id}/{flag}")
    @ResponseBody
    public JsonUtil updateLeave(WanganUserLeave leave, @PathVariable String taskId, @PathVariable String id, @PathVariable boolean flag) {
        JsonUtil j = new JsonUtil();
        try {
            WanganUserLeave oldLeave = leaveService.selectByPrimaryKey(leave.getId());
            BeanUtil.copyNotNullBean(leave, oldLeave);
            leaveService.updateByPrimaryKeySelective(oldLeave);

            Map<String, Object> variables = taskService.getVariables(taskId);
//            WanganUserLeave WanganUserLeave = (WanganUserLeave) variables.get("WanganUserLeave");
    ...
    @ResponseBody
    public JsonUtil addLeave(Model model, WanganUserLeave WanganUserLeave) {
        JsonUtil j = new JsonUtil();
        if (WanganUserLeave == null) {
            return JsonUtil.error("获取数据失败");
        }
        WanganUserLeave.setDays(3);
        CurrentUser user = CommonUtil.getUser();
        WanganUserLeave.setUserId(user.getId());
        WanganUserLeave.setUserName(user.getUsername());
        WanganUserLeave.setProcessInstanceId("2018");//模拟数据
        leaveService.insertSelective(WanganUserLeave);
        Map<String, Object> map = new HashMap<>();
        WanganUserLeave.setUrlpath("/wangan/leave/readOnlyLeave/" + WanganUserLeave.getId());
        map.put("baseTask", WanganUserLeave);
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("process", map);
        WanganUserLeave.setProcessInstanceId(processInstance.getId());
        WanganUserLeave WanganUserLeave1 = leaveService.selectByPrimaryKey(WanganUserLeave.getId());
        BeanUtil.copyNotNullBean(WanganUserLeave, WanganUserLeave1);
        WanganUserLeave1.setUrlpath("/wangan/leave/readOnlyLeave/" + WanganUserLeave.getId());
        leaveService.updateByPrimaryKeySelective(WanganUserLeave1);
        j.setMsg("请假申请成功");
        return j;
    }
    ...
    public String readOnlyLeave(Model model, @PathVariable String billId) {
        WanganUserLeave leave = leaveService.selectByPrimaryKey(billId);
        model.addAttribute("leave", leave);
        return "/act/wangan/leave/update-leave-readonly";
    }
    ...
    public String showTaskList(Model model, com.len.entity.Task task, String page, String limit) {
        ...
        for (Task task1 : taskList) {
            ...
            BaseTask WanganUserLeave = (BaseTask) map.get("baseTask");

            taskEntity = new com.len.entity.Task(task1);
            taskEntity.setUserName(WanganUserLeave.getUserName());
            taskEntity.setReason(WanganUserLeave.getReason());
            taskEntity.setUrlpath(WanganUserLeave.getUrlpath());
            /**如果是自己*/
            if (user.getId().equals(WanganUserLeave.getUserId())) {
    ...
    public String agent(Model model, @PathVariable("id") String taskId) {
        Map<String, Object> variables = taskService.getVariables(taskId);
        BaseTask baseTask = (BaseTask) variables.get("baseTask");
//        WanganUserLeave WanganUserLeave = leaveService.selectByPrimaryKey(baseTask.getId());
...
//        }
    ...
    public String shinePics(Model model, @PathVariable String processInstanceId) {
        ...
        return "/act/wangan/leave/shinePics";
    }
